...
def fun_permutation_test(a_df, b_df, metric, title, ax):
    a_list, b_list = list(a_df[metric]), list(b_df[metric])
    a_size, b_size = len(a_list), len(b_list)
    a_mean, b_mean = mean(a_list), mean(b_list)
    diff = b_mean - a_mean
    all_data_list = a_list + b_list
    diff_list = []
    for _ in range(5000):
        all_data_tmp = np.random.permutation(all_data_list)
        a_tmp = all_data_tmp[ :a_size]
        b_tmp = all_data_tmp[a_size: ]
        a_tmp_mean = mean(a_tmp)
        b_tmp_mean = mean(b_tmp)
        diff_tmp = b_tmp_mean - a_tmp_mean
        diff_list = diff_list + [diff_tmp]
    
    ax.hist(diff_list, alpha = 0.5, label = "diff = mean(a')-mean(b')")
    ax.axvline(x=np.percentile(diff_list, 50), label = 'медиана diff')
    ax.axvline(x=diff, label = 'наблюдаемая разница', c = 'red')
    ax.axvline(x=np.percentile(diff_list, 2.5),linestyle = '--')
    ax.axvline(x=np.percentile(diff_list, 97.5),linestyle = '--', label = '95% diff')
